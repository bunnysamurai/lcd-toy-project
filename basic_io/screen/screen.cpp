...
#elif defined(DUMMY_SCREEN)
#include "dummy_driver/screen_impl.hpp"
#include "dummy_driver/screensize.h"
#endif
...
inline constexpr size_t BUFLEN{DISPLAY_WIDTH * DISPLAY_HEIGHT * CONFIGURED_MAX_BPP / 8};

bool g_video_is_initd{false};

template <class T, class U> constexpr void init_to_all_val(T &buf, const U &val)
{
    for (auto &el : buf)
    {
        el = val;
...
constexpr std::array<uint8_t, BUFLEN> init_the_buffer() noexcept
{
...
TileBuffer<DISPLAY_WIDTH, DISPLAY_HEIGHT, 1, BUFLEN> tile_buf_1bpp{frame_buffer};
TileBuffer<DISPLAY_WIDTH, DISPLAY_HEIGHT, 2, BUFLEN> tile_buf_2bpp{frame_buffer};
TileBuffer<DISPLAY_WIDTH, DISPLAY_HEIGHT, 4, BUFLEN> tile_buf_4bpp{frame_buffer};
TileBuffer<DISPLAY_WIDTH, DISPLAY_HEIGHT / 2, 8, BUFLEN> tile_buf_8bpp{frame_buffer};
TileBuffer<DISPLAY_WIDTH / 2, DISPLAY_HEIGHT / 2, 16, BUFLEN> tile_buf_16bpp{frame_buffer};
...
{
    screen_impl::set_format(fmt);
}

Format get_format() noexcept
{
    return screen_impl::get_format();
}
...
bool init(Position virtual_topleft, Dimensions virtual_size, Format format) noexcept
{
    /* check if we are already init'd */
    if (!g_video_is_initd)
    {
        g_video_is_initd = true;
        return screen_impl::init(std::data(frame_buffer), virtual_topleft, virtual_size, format);
    }
    set_format(format);
...
void draw_tile(uint32_t xpos, uint32_t ypos, Tile tile)
{
    ...
    {
    case screen::Format::GREY1:
        tile_buf_1bpp.draw(tile, xpos, ypos);
        break;
    case screen::Format::GREY2:
        tile_buf_2bpp.draw(tile, xpos, ypos);
        break;
    case screen::Format::GREY4:
    case screen::Format::RGB565_LUT4:
        tile_buf_4bpp.draw(tile, xpos, ypos);
        break;
    case screen::Format::RGB565_LUT8:
        tile_buf_8bpp.draw(tile, xpos, ypos);
        break;
    case screen::Format::RGB565:
        tile_buf_16bpp.draw(tile, xpos, ypos);
        break;
    }
}

void draw_tile_with_replacement(uint32_t xpos, uint32_t ypos, Tile tile, uint32_t pattern, uint32_t replacement)
{
}
...
[[nodiscard]] static constexpr uint32_t compute_column_byte_offset(const screen::Format fmt,
...
[[nodiscard]] static constexpr uint8_t *get_start_of_row(uint8_t *p_start_of_frame, const screen::Format fmt,
...
void fillrows(uint32_t value, uint32_t row_start, uint32_t row_finish, uint32_t column_start, uint32_t column_finish)
{
    ...
    column_start = {column_start > dims.width ? 0 : column_start}; /* clamp a negative column request */
    ...
    auto *p_row{get_start_of_row(screen::get_video_buffer(), fmt, row_start, dims)};
    const auto *const p_row_end{get_start_of_row(screen::get_video_buffer(), fmt, row_finish, dims)};
    ...
    else
    {
        ...
        {
        case screen::Format::GREY1:
            expanded = (value & 0b1) | (value & 0b1) << 1 | (value & 0b1) << 2 | (value & 0b1) << 3 |
                       (value & 0b1) << 4 | (value & 0b1) << 5 | (value & 0b1) << 6 | (value & 0b1) << 7;
            break;
        case screen::Format::GREY2:
            expanded = (value & 0b11) | (value & 0b11) << 2 | (value & 0b11) << 4 | (value & 0b11) << 6;
            break;
        case screen::Format::GREY4:
        case screen::Format::RGB565_LUT4:
...
void copyrow(const uint32_t dst, const uint32_t src, uint32_t column_start, uint32_t column_finish)
{
    const auto dims{get_virtual_screen_size()};
    ...
    const auto start{compute_column_byte_offset(screen::get_format(), column_start)};
    const auto finish{compute_column_byte_offset(screen::get_format(), column_finish)};

    auto *p_dst{get_start_of_row(screen::get_video_buffer(), screen::get_format(), dst, dims)};
    auto *p_src{get_start_of_row(screen::get_video_buffer(), screen::get_format(), src, dims)};
...
static constexpr ConsoleConfig g_console_cfg{.columns = screen_impl::PHYSICAL_WIDTH_PIXELS / glyphs::tile::width(),
...
{
    const bool status{init({.row = 0, .column = 0}, get_physical_screen_size(), screen::Format::GREY1)};
    if (!status)
    {
        return status;
    }
