 ...
 */
static inline void handle_horizontal_line_case(uint32_t xstart, uint32_t xstop, uint32_t ypos, uint32_t value) noexcept
{
...
static inline void handle_vertical_line_case(uint32_t ystart, uint32_t ystop, uint32_t xpos, uint32_t value) noexcept
{
    while (ystart != ystop)
    {
...
void draw_rect(Rect r, uint32_t value, uint16_t thickness) noexcept
{
    if (thickness == 0)
    {
        screen::fillrows(value, r.topleft.y, r.topleft.y + r.size.height, r.topleft.x, r.topleft.x + r.size.width);
        return;
    }
    ...
    const Point botright{.x = r.topleft.x + r.size.width - 1U, .y = r.topleft.y + r.size.height - 1U};
...
static inline void draw_circle_filled(Point center, uint16_t radius, uint32_t value) noexcept
{
...
static inline void draw_circle_outline_only(Point center, uint16_t radius, uint32_t value) noexcept
{
