...
private:
  // TODO make build-time configurable, also, letters' init method cares about
  // this, so we need to make changes in two places manually right now.
  static constexpr auto use_native_ordering{false};
  static constexpr auto use_rotated_ordering{!use_native_ordering};

  static constexpr auto MAX_CHARACTER_COLUMN_COUNT{
  ...
  constexpr void increment_row_native() {
    if (line == MAX_ROW_COUNT - 1) {
      scroll_up(
          buffer,
          8); // TODO magic number, it's the number of native rows to scroll,
              // which is a function of the video buffer and the tiles used.
    } else {
      ++line;
    }
  }
  ...
  constexpr void decrement_column_rotated() {
    // when we decrement a "column" position, we actually increment the line
    // position
    if (line == MAX_ROW_COUNT - 1) {
      jump_to_previous_row();
    } else {
  ...
  constexpr void increment_column_rotated() {
    ...
    // when we increment a "column" position, we actually decrement the line
    // position if the current line position is zero, we
  ...
  constexpr void increment_row_rotated() {
    ...
    // if current column is one-less-the-max, reset to zero.  This effectively
    // increments the "row"
    if (column == MAX_CHARACTER_COLUMN_COUNT - 1) {
      scroll_left(buffer, 1); // TODO magic number, this is the number of native
                              // columns to scrll, which is a function of the
                              // video buffer and the tiles used.
    } else {
      ++column;
    }
  }
  ...
  template <class Tile> constexpr Tile adjust_tile(Tile tile) {
    if constexpr (use_native_ordering) {
      return tile;
    } else if (use_rotated_ordering) {
...
public:
  ...
  friend constexpr void print(TextOut &dev, const char *str) {
  ...
  template <size_t N>
  friend constexpr void print(TextOut &dev, const char (&str)[N]) {
  ...
  friend constexpr void print(TextOut &dev, const char c) {
    if (!check_if_null(c)) {
      putc(dev, c);
    }
  }
