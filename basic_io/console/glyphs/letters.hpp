...
struct LetterType {
  ...
  [[nodiscard]] constexpr const uint8_t &operator[](size_t ii) const {
    return m_data[ii];
  }
...
/**
 * @brief I want flexibility to specify where LetterTypes live in memory. Hence,
 * this function to abstract that away.
 ...
 */
[[nodiscard]] inline constexpr const LetterType &decode_ascii(char c) noexcept {
  // alpha numeric for now
  // for the magic number (or expression, such as it is), see the table at the
  // top of:
  //      https://en.wikipedia.org/wiki/ASCII
  return g_letter_list[decode(c)];
}

namespace tile {

/** @brief Width and height of letter, in pixels */
[[nodiscard]] constexpr size_t width() noexcept {
  return g_letter_list[decode('_')].width_pixels;
}
[[nodiscard]] constexpr size_t height() noexcept {
  return g_letter_list[decode('_')].height_pixels;
}
...
[[nodiscard]] inline Tile decode_ascii(char c) noexcept {
  ...
  return {.side_length = letter.width_pixels,
          .format = screen::Format::GREY1,
          .data = std::data(letter.m_data)};
}
} // namespace tile
} // namespace glyphs
