    ...
    for (uint8_t i = 0; i < 6; i++)
    {
        if (report->keycode[i])
        {
            ...
            else
            {
                ...
                g_tinyusb_key.push_char(keycode2ascii[report->keycode[i]][is_shift ? 1 : 0]);
...
namespace keyboard::bsp
{
    ...
    [[nodiscard]] char TinyUsb_BasicKeyboard::wait_key(duration timeout, result_t &err) noexcept
    {
    ...
    void TinyUsb_BasicKeyboard::push_char(char c) noexcept
    {
        current_char = c;
        received = true;
    }
