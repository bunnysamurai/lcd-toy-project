...
{
#ifdef STOPWATCH_IS_READY
    g_process_stopwatch_timer.period(PROCESS_STOPWATCH_PERIOD_US); /* aka 1 second */
...
[[nodiscard]] Grid::Location find_suitable_cat_spawn() noexcept
{
    ...
    static_assert(GRID_SIZE_ROWS == GRID_SIZE_COLS,
    ...
    auto &&generate_section_selection_offsets{[](auto &randnum) {
        ...
        const auto offset{static_cast<uint16_t>((GRID_SIZE_COLS - 3) * ((section_selection >> 1) & 0b1))};
    ...
    for (;;)
    {
        ...
        for (uint32_t ii = 0; ii < NUMBER_OF_INNER_TRIES; ++ii)
        {
            ...
            const auto [offset, x_is_prime]{generate_section_selection_offsets(number)};
            const auto within_selection_index{generate_prime_dimension_index(number)};

            const Grid::Location potential{.x = offset * x_is_prime + within_selection_index * x_is_prime +
            ...
            const bool no_cat_here{std::end(g_cats) ==
                                   std::find_if(std::begin(g_cats), std::end(g_cats),
                                                [=](const auto &beast) { return beast.location() == potential; })};

            if (no_cat_here &&
                static_cast<GridObject>(g_playfield.get(potential.x, potential.y)) == GridObject::NOTHING)
            {
                return potential;
            }
        }
...
[[nodiscard]] constexpr Collision check_for_collision(Grid::Location proposed) noexcept
{
    ...
    const auto type{static_cast<GridObject>(g_playfield.get(proposed.x, proposed.y))};
 ...
 */
[[nodiscard]] bool traverse_moveable_blocks(Grid::Location start, const Direction dir,
                                            Grid::Location &point_of_encounter) noexcept
{
     ...
     */
    static constexpr Grid::Location LIMITS{.x = GRID_SIZE_COLS, .y = GRID_SIZE_ROWS};
    point_of_encounter = start;
    if (point_of_encounter >= LIMITS)
    {
        return false;
    }
    GridObject obj{static_cast<GridObject>(g_playfield.get(point_of_encounter.x, point_of_encounter.y))};
    while (obj == GridObject::MOVABLE_BLOCK)
    {
        ...
        obj = static_cast<GridObject>(g_playfield.get(point_of_encounter.x, point_of_encounter.y));
    }

    return obj == GridObject::CHEESE || obj == GridObject::HOLE || obj == GridObject::NOTHING;
}
...
[[nodiscard]] Collision move_mouse(UserInput request, Beast &beast) noexcept
{
    ...
    case Collision::BLOCK:
        if (traverse_moveable_blocks(proposed_location, dir, encounter))
        {
            ...
            const GridObject object_encountered{g_playfield.get(encounter.x, encounter.y)};

            const bool it_was_a_cat{(object_encountered == GridObject::NOTHING) &&
                                    check_for_collision(encounter) == Collision::CAT};
            const bool the_cat_can_move{check_for_collision(move(encounter, dir)) == Collision::NONE};
            ...
            if (!it_was_a_cat || (it_was_a_cat && the_cat_can_move))
            {
                ...
                {
                    g_playfield.set(static_cast<uint8_t>(GridObject::MOVABLE_BLOCK), encounter.x, encounter.y);
...
[[nodiscard]] constexpr std::pair<Direction, Direction> disposition_a_fuzzy_move(Grid::Location mouse,
                                                                                 Grid::Location cat) noexcept
{
    const int32_t xdiff{static_cast<int32_t>(cat.x) - static_cast<int32_t>(mouse.x)};
    const int32_t ydiff{static_cast<int32_t>(cat.y) - static_cast<int32_t>(mouse.y)};

    const bool x_is_positive{xdiff > 0};
    const bool y_is_positive{ydiff > 0};

    const bool x_is_greater{(xdiff * (-1 * !x_is_positive)) > (ydiff * (-1 * !y_is_positive))};

    const auto xypolar{static_cast<uint8_t>((y_is_positive << 1) | x_is_positive)};
    if (x_is_greater)
    {
        switch (xypolar)
        {
...
[[nodiscard]] bool move_cats()
{
    ...
    */
    ...
    for (auto &cat : g_cats)
    {
        ...
        /* finally, run pathing */
        const auto dir{[&]() {
            const auto [dir1, dir2]{disposition_a_fuzzy_move(g_mouse.location(), cat.location())};
            if (randnum & 0b1)
            {
                return dir1;
            }
    ...
    */
    if (sitting_cats_count == std::size(g_cats))
    {
        for (auto cat : g_cats)
        {
            ...
            g_playfield.set(static_cast<uint8_t>(GridObject::CHEESE), cat.location().x, cat.location().y);
        }
        g_cats.clear();
        add_cats_to_play(2);
#ifdef STOPWATCH_IS_READY
...
[[nodiscard]] UserInput process_user_input() noexcept
{
    ...
    if (!key_pressed.up && !key_pressed.down && !key_pressed.left && !key_pressed.right && !key_pressed.etc)
    {
...
{
    static constexpr std::array LIST_OF_NAMES{"Ben", "Mark", "Tim", "Noah", "Gabe", "Christian"};
    ...
    for (uint32_t yy = ROWOFF; yy < dims.height - ROWOFF; ++yy)
    {
        ...
        for (uint32_t linidx = rowstart + COLOFF; linidx < rowstart + (dims.width - COLOFF); linidx += 2)
        {
    ...
    const screen::Tile tile_obj{
        .side_length = glyphs::tile::width(), .format = screen::Format::RGB565_LUT4, .data = std::data(buffer)};
    auto &&draw_name{[&](const char *str, const uint32_t row_start, const uint32_t col_start) {
    ...
    for (uint32_t idx = 0; idx < std::size(rolls); ++idx)
    {
        ...
        draw_name(LIST_OF_NAMES[nameidx], ROWOFF + 5 + idx * glyphs::tile::width(), COLOFF + 5);
    }
...
void draw_lives(uint8_t life_count) noexcept
{
    ...
    if (!screen::copy_with_replacement(MOUSE, mouse_with_grey_background, BKGRND, LGREY))
    {
        printf("Unable to draw lives??\n");
    }

    /* clear the whole thing */
    screen::fillrows(LGREY, g_top_panel.lives_draw_start.y, g_top_panel.lives_draw_finish.y,
...
void draw_score(uint32_t score)
{
    ...
    {
        const auto tile_1bpp{glyphs::tile::decode_ascii(static_cast<char>(digit + 0x30))};
...
#ifdef STOPWATCH_IS_READY
void draw_stopwatch(Timer_t::time_diff_t amount_on_the_clock, Timer_t::time_diff_t timeout_time)
{
    ...
    const screen::gfx::Point center{.x = g_top_panel.timer_center.x, .y = g_top_panel.timer_center.y};
...
/*                                                                           */
/* ========================================================================= */
void draw_playgrid_border(const Grid &grid, const screen::Tile border_tile) noexcept
{
    ...
    const auto grid_finishpix_xy{grid.to_native({.x = GRID_SIZE_COLS - 1, .y = GRID_SIZE_ROWS - 1})};
    ...
    /* draw top border */
    for (uint32_t col = start_col; col <= finish_col; col += border_tile.side_length)
    {
    ...
    /* draw left border */
    for (uint32_t row = start_row + border_tile.side_length; row <= finish_row - border_tile.side_length;
...
void game_init() noexcept
{
    ...
    g_grid = Grid{Grid::GridCfg{
        .xdimension = {.off = (dims.width - GRID_SIZE_COLS * PIXELS_PER_GRID) >> 1, .scale = PIXELS_PER_GRID},
        .ydimension = {.off = (dims.height - GRID_SIZE_ROWS * PIXELS_PER_GRID) >> 1, .scale = PIXELS_PER_GRID},
    ...
    */
    ...
    g_top_panel =
        TopPanelCfg{.lives_draw_start = {.x = static_cast<uint16_t>(PIXELS_PER_GRID >> 1),
                                         .y = static_cast<uint16_t>((top_panel_height >> 1) - PIXELS_PER_GRID)},
                    .lives_draw_finish = {.x = static_cast<uint16_t>((PIXELS_PER_GRID >> 1) + PIXELS_PER_GRID * 6),
                    ...
                    .score_start = {.x = static_cast<uint16_t>(
                                        top_panel_width - (6 * glyphs::tile::width() + (glyphs::tile::width() >> 1))),
                                    .y = static_cast<uint16_t>((top_panel_height >> 1) - PIXELS_PER_GRID)}};
...
void respawn_mouse()
{
    ...
    g_playfield.set(static_cast<uint8_t>(GridObject::NOTHING), GRID_SIZE_COLS >> 1, GRID_SIZE_ROWS >> 1);
    draw_beast(g_mouse, MOUSE);
}
    ...
    for (;;)
    {
        ...
        if (request != UserInput::NO_ACTION && !g_stuck_in_hole)
        {
            ...
            if (location_before_move != g_mouse.location())
            {
                ...
                g_playfield.set(static_cast<uint8_t>(GridObject::NOTHING), location_before_move.x,
                                location_before_move.y);
                draw_beast(g_mouse, g_stuck_in_hole ? MOUSE_IN_HOLE : MOUSE);
                draw_grid_tile(location_before_move.x, location_before_move.y, BACKGROUND);
            }
        }
