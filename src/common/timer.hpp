...
template <class TimerDetails> class Timer final
{
  public:
    ...
    explicit constexpr Timer(time_diff_t period) : m_period{period}, m_last_touch{TimerDetails::now()}
    {
    }
    ...
    {
        m_last_touch = TimerDetails::now();
    }

    constexpr void increment() noexcept
    {
        m_last_touch = TimerDetails::increment_time_point(m_last_touch, m_period);
    };
