...
template <size_t N> [[nodiscard]] constexpr auto concat(std::array<uint8_t, N> data) noexcept
{
    return data;
}

template <size_t M, size_t... Sizes>
[[nodiscard]] constexpr std::array<uint8_t, M + (Sizes + ...)> concat(std::array<uint8_t, M> lhs,
...
template <class... Args>
[[nodiscard]] constexpr std::array<uint8_t, (sizeof...(Args) + 1) / 2> pfold(Args... args) noexcept
{
...
static_assert(std::equal(std::begin(even_expect), std::end(even_expect), std::begin(even_result)));
...
static_assert(std::equal(std::begin(odd_expect), std::end(odd_expect), std::begin(odd_result)));
} // namespace constexpr_tests
