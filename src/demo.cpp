...
static constexpr auto gold_data{
    embp::pfold(YELLOW, YELLOW, YELLOW, YELLOW, YELLOW, YELLOW, YELLOW, YELLOW, YELLOW, YELLOW, YELLOW, YELLOW, YELLOW,
...
static constexpr auto emerald_data{
    embp::pfold(CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN,
                CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN,
                CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN,
                CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN, CYAN)};
static constexpr auto red_data{embp::pfold(
...
static constexpr auto green_data{embp::pfold(
    ...
    GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN,
    GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN, GREEN)};
static constexpr auto blue_data{
    embp::pfold(BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE, BLUE,
...
constexpr screen::Tile blue{.side_length = 8, .format = screen::Format::RGB565_LUT4, .data = std::data(blue_data)};
constexpr screen::Tile green{.side_length = 8, .format = screen::Format::RGB565_LUT4, .data = std::data(green_data)};
constexpr screen::Tile red{.side_length = 8, .format = screen::Format::RGB565_LUT4, .data = std::data(red_data)};
constexpr screen::Tile magenta{
    .side_length = 8, .format = screen::Format::RGB565_LUT4, .data = std::data(magenta_data)};
constexpr screen::Tile gold{.side_length = 8, .format = screen::Format::RGB565_LUT4, .data = std::data(gold_data)};
constexpr screen::Tile emerald{
    .side_length = 8, .format = screen::Format::RGB565_LUT4, .data = std::data(emerald_data)};

void run_color_rando_art() noexcept
{
    gamepad::five::init();

    static constexpr std::array<screen::Tile, 4> tilelist{gold, green, gold, gold};
    const auto dims{screen::get_virtual_screen_size()};

    auto &&random_routine{[&](uint32_t rando_select) {
        if (rando_select == 0)
        {
            for (size_t yy = 0; yy < (dims.height >> 1); yy += tilelist[0].side_length)
            {
                for (size_t xx = 0; xx < (dims.width >> 1); xx += tilelist[0].side_length)
                {
                    ...
                    screen::draw_tile(dims.width - xx - rtile.side_length, dims.height - yy - rtile.side_length, rtile);
                }
            }
        }
        else if (rando_select == 1)
        {
            ...
            {
                screen::gfx::draw_line({.x = ii, .y = ii}, {.x = ii + 50, .y = ii}, color, 1 + (r >> 2));
                screen::gfx::draw_line({.x = ii, .y = ii}, {.x = ii, .y = ii + 50}, color, 1 + (r >> 2));
            }

            /* test all 8 octets */
            screen::gfx::draw_line({.x = 100, .y = 100}, {.x = 175, .y = 110}, color, 1);
            screen::gfx::draw_line({.x = 100, .y = 100}, {.x = 175, .y = 90}, color, 1);
            screen::gfx::draw_line({.x = 100, .y = 100}, {.x = 110, .y = 175}, color, 1);
            screen::gfx::draw_line({.x = 100, .y = 100}, {.x = 110, .y = 25}, color, 1);

            screen::gfx::draw_line({.x = 100, .y = 100}, {.x = 25, .y = 110}, color, 1);
            screen::gfx::draw_line({.x = 100, .y = 100}, {.x = 25, .y = 90}, color, 1);
            screen::gfx::draw_line({.x = 100, .y = 100}, {.x = 90, .y = 175}, color, 1);
            screen::gfx::draw_line({.x = 100, .y = 100}, {.x = 90, .y = 25}, color, 1);
        }
    }};
...
void run_linebounce_screensaver() noexcept
{
    ...
    std::array position{
        ...
        /* outer */
        Position{.x = (10 << 14) + sf * 10000, .y = (11 << 14) + sf * 11000},
        Position{.x = (20 << 14) - sf * 10000, .y = (300 << 14) - sf * 11000},
        Position{.x = (200 << 14) + sf * 10000, .y = (300 << 14) + sf * 11000},
        Position{.x = (200 << 14) - sf * 10000, .y = (30 << 14) - sf * 11000},
    ...
    std::array velocity{
        Velocity{.dx = 10000, .dy = 11000}, Velocity{.dx = -10000, .dy = -11000},
        Velocity{.dx = 10000, .dy = 11000}, Velocity{.dx = -10000, .dy = -11000},
...
[[nodiscard]] screen::TouchReport to_pixelspace(screen::TouchReport rawloc) noexcept
{
    /*
    ...
    const auto raw_x{(M_COL_S_24_8 * static_cast<int32_t>(rawloc.x) + B_COL_S_24_8) >> 8};
    const auto raw_y{(M_ROW_S_24_8 * static_cast<int32_t>(rawloc.y) + B_ROW_S_24_8) >> 8};

    const auto dims{screen::get_virtual_screen_size()};
    return screen::TouchReport{
        .x = zclamp(raw_x, dims.width - 1), .y = zclamp(raw_y, dims.height - 1), .pen_up = rawloc.pen_up};
}
...
void undo_cool_touch_action(screen::TouchReport touch_loc) noexcept
{
    ...
    const auto dims{screen::get_virtual_screen_size()};
    screen::draw_tile(dims.width - touch_loc.x - 1, dims.height - touch_loc.y - 1, emerald);
}
...
void take_cool_touch_action(screen::TouchReport touch_loc) noexcept
{
    touch_loc = to_pixelspace(touch_loc);
    const auto dims{screen::get_virtual_screen_size()};
    screen::draw_tile(dims.width - touch_loc.x - 1, dims.height - touch_loc.y - 1, red);
}

} // namespace

static constexpr int64_t DOUBLE_TAP_PERIOD_THRESHOLD_US{550 * 1000}; /* 100 ms? */
