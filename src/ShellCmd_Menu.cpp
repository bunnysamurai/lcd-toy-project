...
MenuConfig g_cfg{.startcol = 1, .startline = 3, .row_spacing = 2, .titlestartline = 1};
...
std::array<std::pair<const char *, void (*)(void)>, 6> g_f_table{
    ...
    std::make_pair("Claire's Challenge",
                   [] {
                       chippie::run();
                       sleep_ms(100);
                   }),
    ...
    std::make_pair("Exit to Shell", [] { g_keep_running = false; }) /* keep this one at the bottom */};
